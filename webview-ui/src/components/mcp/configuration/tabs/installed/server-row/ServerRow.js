...
var ServerRow = function (_a) {
	...
	var server = _a.server,
		_c = _a.isExpandable,
		isExpandable = _c === void 0 ? true : _c
	var _d = (0, ExtensionStateContext_1.useExtensionState)(),
		mcpMarketplaceCatalog = _d.mcpMarketplaceCatalog,
		autoApprovalSettings = _d.autoApprovalSettings
	...
	var _g = (0, react_1.useState)(function () {
			...
			try {
				...
				return (
					((_a = config.timeout) === null || _a === void 0 ? void 0 : _a.toString()) ||
					mcp_1.DEFAULT_MCP_TIMEOUT_SECONDS.toString()
				)
			} catch (_b) {
	...
	var handleAutoApproveChange = function () {
		...
		vscode_1.vscode.postMessage({
			...
			serverName: server.name,
			toolNames:
				((_a = server.tools) === null || _a === void 0
					? void 0
					: _a.map(function (tool) {
							return tool.name
						})) || [],
			autoApprove: !((_b = server.tools) === null || _b === void 0
				? void 0
				: _b.every(function (tool) {
				...
				isExpanded && (
					<div
						...
						<react_2.VSCodePanels>
							<react_2.VSCodePanelTab id="tools">
								Tools ({((_b = server.tools) === null || _b === void 0 ? void 0 : _b.length) || 0})
							</react_2.VSCodePanelTab>
							<react_2.VSCodePanelTab id="resources">
								Resources (
								{__spreadArray(
									__spreadArray([], server.resourceTemplates || [], true),
									server.resources || [],
									true,
								).length || 0}
								)
								...
								{server.tools && server.tools.length > 0 ? (
									<div
										...
										{server.tools.map(function (tool) {
											return <McpToolRow_1.default key={tool.name} tool={tool} serverName={server.name} />
										})}
										{server.name && autoApprovalSettings.enabled && autoApprovalSettings.actions.useMcp && (
											<react_2.VSCodeCheckbox
												style={{ marginBottom: -10 }}
												checked={server.tools.every(function (tool) {
													return tool.autoApprove
												})}
						...
						<div style={{ margin: "10px 7px" }}>
							...
							<react_2.VSCodeDropdown style={{ width: "100%" }} value={timeoutValue} onChange={handleTimeoutChange}>
								{timeoutOptions.map(function (option) {
									return (
										<react_2.VSCodeOption key={option.value} value={option.value}>
											{option.label}
