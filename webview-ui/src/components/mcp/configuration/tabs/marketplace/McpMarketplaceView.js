...
var McpMarketplaceView = function () {
    ...
    var _f = (0, react_1.useState)(null), selectedCategory = _f[0], setSelectedCategory = _f[1];
    var _g = (0, react_1.useState)("newest"), sortBy = _g[0], setSortBy = _g[1];
    var categories = (0, react_1.useMemo)(function () {
        var uniqueCategories = new Set(items.map(function (item) { return item.category; }));
    ...
    var filteredItems = (0, react_1.useMemo)(function () {
        return items
            ...
            var matchesSearch = searchQuery === "" ||
                ...
                item.tags.some(function (tag) { return tag.toLowerCase().includes(searchQuery.toLowerCase()); });
    ...
    if (error) {
        return (<div style={{
				...
				<react_2.VSCodeButton appearance="secondary" onClick={function () { return fetchMarketplace(true); }}>
    ...
    return (<div style={{
			...
			<div style={{ padding: "20px 20px 5px", display: "flex", flexDirection: "column", gap: "16px" }}>
				{/* Search row */}
				<react_2.VSCodeTextField style={{ width: "100%" }} placeholder="Search MCPs..." value={searchQuery} onInput={function (e) { return setSearchQuery(e.target.value); }}>
					<div slot="start" className="codicon codicon-search" style={{
            fontSize: 13,
            opacity: 0.8,
        }}/>
					{searchQuery && (<div className="codicon codicon-close" aria-label="Clear search" onClick={function () { return setSearchQuery(""); }} slot="end" style={{
                display: "flex",
                justifyContent: "center",
                alignItems: "center",
                height: "100%",
            ...
            flex: 1,
						...
						<react_2.VSCodeDropdown style={{
            width: "100%",
        }} value={selectedCategory || ""} onChange={function (e) { return setSelectedCategory(e.target.value || null); }}>
							<react_2.VSCodeOption value="">All Categories</react_2.VSCodeOption>
							{categories.map(function (category) { return (<react_2.VSCodeOption key={category} value={category}>
									{category}
            ...
            marginTop: "3px",
					...
					<react_2.VSCodeRadioGroup style={{
            display: "flex",
            flexWrap: "wrap",
            marginTop: "-2.5px",
        }} value={sortBy} onChange={function (e) { return setSortBy(e.target.value); }}>
			...
			<div style={{ display: "flex", flexDirection: "column" }}>
				{filteredItems.length === 0 ? (<div style={{
                display: "flex",
                justifyContent: "center",
                alignItems: "center",
                height: "100%",
                ...
                : "No MCP servers found in the marketplace"}
					</div>) : (filteredItems.map(function (item) { return <McpMarketplaceCard_1.default key={item.mcpId} item={item} installedServers={mcpServers}/>; }))}
