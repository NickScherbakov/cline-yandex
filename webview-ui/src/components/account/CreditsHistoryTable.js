...
var CreditsHistoryTable = function (_a) {
	var isLoading = _a.isLoading,
		usageData = _a.usageData,
		paymentsData = _a.paymentsData
	var _b = (0, react_2.useState)("usage"),
		activeTab = _b[0],
		setActiveTab = _b[1]
	return (
				...
				{isLoading ? (
					...
					<>
                        ...
                        Tokens Used
										...
										{usageData.map(function (row, index) {
											return (
												<react_1.VSCodeDataGridRow key={index}>
													...
													<react_1.VSCodeDataGridCell grid-column="2">
														{"".concat(row.modelProvider, "/").concat(row.model)}
													</react_1.VSCodeDataGridCell>
													...
													<react_1.VSCodeDataGridCell grid-column="3">
														{"$".concat(Number(row.credits).toFixed(7))}
													</react_1.VSCodeDataGridCell>
												</react_1.VSCodeDataGridRow>
											)
										})}
							...
							<>
								{paymentsData.length > 0 ? (
									<react_1.VSCodeDataGrid>
										...
										{paymentsData.map(function (row, index) {
											return (
												<react_1.VSCodeDataGridRow key={index}>
													<react_1.VSCodeDataGridCell grid-column="1">
													...
													<react_1.VSCodeDataGridCell grid-column="2">
														{"$".concat((0, format_1.formatDollars)(parseInt(row.amountCents)))}
													</react_1.VSCodeDataGridCell>
													<react_1.VSCodeDataGridCell grid-column="3">
														{"".concat(row.credits)}
													</react_1.VSCodeDataGridCell>
