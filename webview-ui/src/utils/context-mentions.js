...
function getContextMenuOptions(query, selectedType, queryItems, dynamicSearchResults) {
    ...
    if (dynamicSearchResults === void 0) { dynamicSearchResults = []; }
    var workingChanges = {
    ...
    if (query === "") {
        if (selectedType === ContextMenuOptionType.File) {
            var files = queryItems
                .filter(function (item) { return item.type === ContextMenuOptionType.File; })
        ...
        if (selectedType === ContextMenuOptionType.Folder) {
            var folders = queryItems
                .filter(function (item) { return item.type === ContextMenuOptionType.Folder; })
                .map(function (item) { return ({
        ...
        if (selectedType === ContextMenuOptionType.Git) {
            var commits = queryItems.filter(function (item) { return item.type === ContextMenuOptionType.Git; });
    ...
    if (/^[a-f0-9]{7,40}$/i.test(lowerQuery)) {
        var exactMatches = queryItems.filter(function (item) { var _a; return item.type === ContextMenuOptionType.Git && ((_a = item.value) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === lowerQuery; });
    ...
    var searchableItems = queryItems.map(function (item) { return ({
        original: item,
    ...
    var matchingItems = query ? fzf.find(query).map(function (result) { return result.item.original; }) : [];
    // Separate matches by type
    var fileMatches = matchingItems.filter(function (item) { return item.type === ContextMenuOptionType.File || item.type === ContextMenuOptionType.Folder; });
    var gitMatches = matchingItems.filter(function (item) { return item.type === ContextMenuOptionType.Git; });
    ...
    if (suggestions.length > 0 || matchingItems.length > 0) {
        var allItems = __spreadArray(__spreadArray(__spreadArray(__spreadArray([], suggestions, true), fileMatches, true), gitMatches, true), otherMatches, true);
