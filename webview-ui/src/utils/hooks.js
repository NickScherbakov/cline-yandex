...
var useMetaKeyDetection = function (platform) {
	var _a = (0, react_1.useState)(platformUtils_1.unknown),
		metaKeyChar = _a[0],
		setMetaKeyChar = _a[1]
	var _b = (0, react_1.useState)(platformUtils_1.unknown),
		os = _b[0],
		setOs = _b[1]
	;(0, react_1.useEffect)(
		function () {
		...
		function (event) {
			...
			if (shortcut.includes("+")) {
				...
				if (Object.keys(modifierMap).includes(keyArray[0])) {
					...
					if (
						keyArray.every(function (k) {
							return modifierMap[k]
						}) &&
					...
					if (keyArray[keyCombo.length] === event.key) {
						...
						return setKeyCombo(function (prevCombo) {
							return __spreadArray(__spreadArray([], prevCombo, true), [event.key], false)
						})
					}
