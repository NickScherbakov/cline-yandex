...
describe("YandexCloudHandler", () => {
	...
	const mockMessages: Message[] = [{ role: "user", content: "Привет, мир!" }]
	...
	describe("createMessage", () => {
		...
		it("должен выбросить ошибку при попытке потоковой передачи для неподдерживающей модели", async () => {
			...
			await expect(handler.createMessage("Ты - ассистент", mockMessages, { stream: true })).rejects.toThrow(
		...
		it("должен обрабатывать ошибки API корректно", async () => {
			...
			const handler = new YandexCloudHandler(mockOptions)
			await expect(handler.createMessage("Ты - ассистент", mockMessages)).rejects.toThrow(
				"Ошибка API YandexCloud: Ошибка авторизации",
			)
		})
	})
})
