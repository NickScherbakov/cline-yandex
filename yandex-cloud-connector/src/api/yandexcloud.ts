...
export class YandexCloudHandler {
	 ...
	 */
	async createMessage(systemPrompt: string | undefined, messages: Message[], options: Partial<YandexCloudRequestOptions> = {}) {
	 ...
	 */
	private async createStandardRequest(messages: Message[], options: YandexCloudRequestOptions): Promise<YandexCloudResponse> {
		const endpoint =
			options.modelId === "summarization" ? YANDEX_CLOUD_ENDPOINTS.TEXT_GENERATION : YANDEX_CLOUD_ENDPOINTS.COMPLETION
		...
		try {
			...
			if (axios.isAxiosError(error)) {
				throw new Error(`Ошибка API YandexCloud: ${error.response?.data?.message || error.message}`)
			}
			throw error
		}
	}
	...
	private async *createStreamingRequest(
		...
		try {
			...
			if (axios.isAxiosError(error)) {
				throw new Error(`Ошибка потокового API YandexCloud: ${error.response?.data?.message || error.message}`)
			}
			throw error
		}
	}
